...
(function() {
  ...
  function injectLoginFields() {
    ...
    if (loginCardHasUsernameField(card)) {
      ...
      // Remove campos de telefone injetados pelo patch - manter apenas o original
      var injected = document.querySelectorAll('[data-a73-login-account-field]');
    ...
    userItem.className = 'item-height';
    userItem.style.cssText = 'margin-bottom:8px;display:block !important;visibility:visible !important;';
    userItem.innerHTML = '<ion-label position="stacked">Número de telefone</ion-label><ion-input type="tel" placeholder="Ex: (11) 99999-9999" data-a73-login-account-input inputmode="numeric"></ion-input>';
  ...
  function applyRegisterSuccessAndLogin(data, account) {
    try {
      ...
      if (d && d.token) {
        ...
        localStorage.setItem('account', (d.user && d.user.account) || account || '');
      ...
      try {
        ...
        function patchInput(inp, isLogin, isReg) {
          ...
          if (isLogin) {
            ...
            if (acc) inp.account = inp.username = inp.phone = acc;
